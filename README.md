...
Скрипт `predev` автоматически проверит файлы проекта, создаст `llm.config.js` при необходимости и сгенерирует данные курсов, если JSON-файлов нет.
...
После изменения исходных таблиц обновите данные:

```bash
npm run data
```

Скрипт собирает оба курса: математику и Финграм.
