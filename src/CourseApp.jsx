...
const INTRO_MARKDOWN_COMPONENTS = {
...
const TREE_INDENT_PX = 4;

function collectExpandableIds(forest) {
  const ids = new Set();
  for (const [id, childIds] of forest.children.entries()) {
    if (childIds.length > 0) ids.add(id);
  }
  return ids;
}
    ...
    <div className={`kb-tree-node ${isSelected ? "kb-tree-node--selected" : ""}`}>
      <div
        className={[
          ...
          .filter(Boolean)
          .join(" ")}
        style={{ paddingLeft: `${2 + depth * TREE_INDENT_PX}px` }}
      >
        {hasChildren ? (
          <button
            type="button"
...
function KnowledgeTree({ selectedId, skills, onSelect }) {
  ...
  const [expanded, setExpanded] = useState(() => collectExpandableIds(forest));

  useEffect(() => {
    setExpanded(collectExpandableIds(forest));
  }, [forest]);
      ...
      <article className="theory-document">
        <div className="theory-document__intro">
          <ReactMarkdown
            remarkPlugins={[remarkMath]}
            rehypePlugins={[rehypeKatex]}
            components={INTRO_MARKDOWN_COMPONENTS}
          >
            {skill.description || "Разберём правило, примеры и способ решения."}
          </ReactMarkdown>
        </div>
        <ReactMarkdown
          key={skill.id}
...
export default function CourseApp({ course, onHome }) {
  const skills = course.data.skills;
  ...
  const initialId = useMemo(() => {
    ...
    return root?.id || course.data.featuredSkillId;
  }, [skills, course.data.featuredSkillId]);
    ...
    return (
      <div className="app-shell">
        ...
        <main className="content">
          <div className="empty-state">
            <BookOpen size={34} />
            <h2>Не удалось открыть навык</h2>
            <p>Попробуйте вернуться на главную и выбрать курс снова.</p>
